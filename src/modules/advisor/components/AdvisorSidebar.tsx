...
export function AdvisorSidebar({ mobileOpen, onCloseMobile }: AdvisorSidebarProps) {
        ...
        {ADVISOR_NAV.map((group) => (
          <div key={group.label} className="mb-3 flex flex-col gap-0.5">
            ...
            {group.items.map((item) => {
              const active =
                item.href === "/advisor" ? pathname === "/advisor" : pathname.startsWith(item.href);
