...
interface CoeTopbarProps {
  user: AuthUser;
  onOpenMobileNav: () => void;
  onLogout: () => void;
}

export function CoeTopbar({ user, onOpenMobileNav, onLogout }: CoeTopbarProps) {
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-medium tracking-wide text-blue-100">
              Sri Eshwar College of Engineering · Controller of Examinations
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden text-right sm:block">
            ...
            <p className="text-xs capitalize text-blue-100">
              {user.isSeniorCoe ? "Senior COE" : user.role}
            </p>
          </div>
          <button
            onClick={onLogout}
