import { ShieldCheckIcon, DocumentIcon, BankIcon, AlertTriangleIcon } from "@/shared/components/icons";
import { formatCurrency } from "../fee-payments/format";
import type { EducationLoanDDSummary } from "./types";
...
export function EducationLoanDDSummaryCard({ summary }: { summary: EducationLoanDDSummary }) {
  return (
    <div className="flex h-full flex-col rounded-[var(--r-xl)] border border-[var(--border-subtle)] bg-white p-4 shadow-[var(--shadow-xs)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[var(--shadow-md)]">
      <div className="flex items-center gap-2.5">
        ...
        <h4 className="text-[13.5px] font-semibold text-[var(--text-primary)]">Education Loan DD Summary</h4>
      </div>

      <div className="mt-4">
        <p className="text-[11.5px] text-[var(--text-tertiary)]">Total DD Amount</p>
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <StatChip label="Received" value={summary.receivedCount} icon={BankIcon} />
        <StatChip label="Cleared" value={summary.clearedCount} icon={ShieldCheckIcon} />
        <StatChip label="Bounced" value={summary.bouncedCount} icon={AlertTriangleIcon} />
      </div>
    </div>
  );
}
