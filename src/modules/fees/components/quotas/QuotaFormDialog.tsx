...
export function QuotaFormDialog({ quota, error, isSubmitting, onClose, onSubmit }: QuotaFormDialogProps) {
  const [name, setName] = useState(quota?.name ?? "");
  const isEdit = quota !== null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/40 p-4">
      <div className="w-full max-w-lg rounded-xl bg-white p-7 shadow-xl">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-zinc-900">{isEdit ? "Edit Quota" : "Add Quota"}</h3>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
        ...
        <form onSubmit={handleSubmit} className="mt-5 flex flex-col gap-5">
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            >
              {isSubmitting ? "Saving..." : isEdit ? "Save Changes" : "Add Quota"}
            </button>
          </div>
        </form>
      </div>
