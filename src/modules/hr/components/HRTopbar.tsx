...
export function HRTopbar({ user, onOpenMobileNav, onLogout }: HRTopbarProps) {
  return (
      ...
      <div className="relative z-10 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden text-right sm:block">
            ...
            <p className="text-xs capitalize text-blue-100">{user.role.replace("_", " ")}</p>
          </div>
          <button
            onClick={onLogout}
            className="flex shrink-0 items-center gap-2 whitespace-nowrap rounded-md border border-white/30 px-3 py-2 text-sm font-medium text-white hover:bg-white/10"
          >
