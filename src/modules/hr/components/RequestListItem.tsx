...
interface RequestListItemProps {
  ...
  selectable?: boolean;
  selected?: boolean;
  onToggleSelect?: (request: HrUnifiedRequest) => void;
}

export function RequestListItem({ request, onOpen, selectable, selected, onToggleSelect }: RequestListItemProps) {
  return (
    <div className="flex w-full items-start gap-3 border-b border-slate-100 px-5 py-4 last:border-b-0 hover:bg-slate-50">
      {selectable && (
        <input
          type="checkbox"
          className="mt-2 h-4 w-4 shrink-0 rounded border-slate-300 text-blue-700 focus:ring-blue-500"
          checked={selected ?? false}
          onChange={() => onToggleSelect?.(request)}
          onClick={(e) => e.stopPropagation()}
          aria-label={`Select request from ${request.faculty.first_name} ${request.faculty.last_name}`}
        />
      )}
      <button onClick={() => onOpen(request)} className="flex w-full flex-col gap-2 text-left">
