...
import { IqacTopbar } from "./IqacTopbar";

const ALLOWED_ROLES = new Set(["iqac", "admin"]);
...
export function IqacShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden bg-slate-50">
      <IqacTopbar user={user} onOpenMobileNav={() => setMobileNavOpen(true)} onLogout={handleLogout} />
      <div className="flex min-h-0 flex-1">
        <IqacSidebar mobileOpen={mobileNavOpen} onCloseMobile={() => setMobileNavOpen(false)} />
