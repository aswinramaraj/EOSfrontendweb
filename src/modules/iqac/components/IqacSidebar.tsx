...
export function IqacSidebar({ mobileOpen, onCloseMobile }: IqacSidebarProps) {
        ...
        {IQAC_NAV.map((group) => (
          <div key={group.label} className="mb-3 flex flex-col gap-0.5">
            ...
            {group.items.map((item) => {
              const active = item.href === "/iqac" ? pathname === "/iqac" : pathname.startsWith(item.href);
    ...
    <>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 flex lg:hidden">
          ...
          <aside className="relative flex h-full w-65 flex-col overflow-y-auto bg-white shadow-xl">{content}</aside>
        </div>
      )}
    </>
  );
