...
interface IqacTopbarProps {
  user: AuthUser;
  onOpenMobileNav: () => void;
  onLogout: () => void;
}

export function IqacTopbar({ user, onOpenMobileNav, onLogout }: IqacTopbarProps) {
  return (
    <header className="relative isolate z-10 overflow-hidden bg-linear-to-r from-blue-800 via-blue-700 to-blue-500 px-6 py-4">
      ...
      <div className="relative z-10 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button onClick={onOpenMobileNav} className="text-white/80 hover:text-white lg:hidden" aria-label="Open menu">
          ...
          />
          <div>
            <h1 className="text-lg font-bold leading-tight text-white sm:text-xl">IQAC Platform</h1>
            <p className="text-xs font-medium tracking-wide text-blue-100">
              University ERP Administration
            </p>
          </div>
        </div>
