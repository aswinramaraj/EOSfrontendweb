...
export const SelectInput = forwardRef<HTMLSelectElement, SelectInputProps>(
  ({ hasError, className = "", children, ...rest }, ref) => (
    <select
      ref={ref}
      className={`${WIDTH_CLASS_PATTERN.test(className) ? "" : "w-full"} rounded-md border bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary-tint ${
