...
export const TextInput = forwardRef<HTMLInputElement, TextInputProps>(
  ({ hasError, className = "", ...rest }, ref) => (
    <input
      ref={ref}
      className={`${WIDTH_CLASS_PATTERN.test(className) ? "" : "w-full"} rounded-md border px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-100 ${
